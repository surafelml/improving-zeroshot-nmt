...
# Script updated from https://github.com/neulab/word-embeddings-for-nmt/blob/master/ted_reader.py
# For full detail of the ted-data version and reader see: https://github.com/neulab/word-embeddings-for-nmt

parser = argparse.ArgumentParser()
#parser.add_argument('--lang-pair', required=False,
#                    help='Comma separated list of languages')
parser.add_argument('--lang-file', required=True,
                    help='File with list of lang pair (en-it, en-ro, etc),'
                         'for extracting parallel data.')
parser.add_argument('--data-path', required=True,
                    help='Downloaded ted data path http://phontron.com/data/ted_talks.tar.gz')
...
if __name__ == "__main__":
    ...
    ted_data_path = args.data_path  # "ted_data"
    lang_pair = args.lang_file

    with open(lang_pair, 'r') as lang_file:
        for lang_pair in lang_file:
            src_lang, trg_lang = lang_pair.strip().split("-")

            # for this experiment make sure the tgt side is english/en
            if src_lang != trg_lang and trg_lang == 'en':
                print("Parallel TedTalks data extraction for {} pair".format(src_lang+"->"+trg_lang))

                output_data_path = os.path.join(ted_data_path, src_lang+"-"+trg_lang)
                ...
                print("Skip extraction for {} pair".format(src_lang+"->"+trg_lang))